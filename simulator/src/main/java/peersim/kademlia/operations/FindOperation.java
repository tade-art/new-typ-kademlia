...
public class FindOperation extends Operation {
  ...
  /**
   * number of available find request message to send (it must be always less than
   * ALPHA)
   */
  protected int available_requests;
  /**
   * This map contains the K closest nodes and corresponding boolean value that
   * indicates if the
  ...
  public List<BigInteger> getNeighboursList() {
    ...
    // return new ArrayList<BigInteger>(closestSet.keySet()).subList(0,
    // KademliaCommonConfig.K-1);
  }
